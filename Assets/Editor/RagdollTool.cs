...
namespace UnityEditor
{
    class RagdollBuilder : ScriptableWizard
    {
        ...
        [MenuItem("GameObject/3D Object/Ragdoll Auto...", false, 2000)]
        static void CreateWizard()
        {
            RagdollBuilder builder = DisplayWizard<RagdollBuilder>("Create Ragdoll");
            GameObject activeGameObject = Selection.activeGameObject;
            if (activeGameObject != null)
            {
                builder.pelvis = activeGameObject.transform.FindChild<Transform>("Bip001 Pelvis", 0);
                builder.leftHips = activeGameObject.transform.FindChild<Transform>("Bip001 L Thigh", 0);
                builder.leftKnee = activeGameObject.transform.FindChild<Transform>("Bip001 L Calf", 0);
                builder.leftFoot = activeGameObject.transform.FindChild<Transform>("Bip001 L Foot", 0);

                builder.rightHips = activeGameObject.transform.FindChild<Transform>("Bip001 R Thigh", 0);
                builder.rightKnee = activeGameObject.transform.FindChild<Transform>("Bip001 R Calf", 0);
                builder.rightFoot = activeGameObject.transform.FindChild<Transform>("Bip001 R Foot", 0);

                builder.leftArm = activeGameObject.transform.FindChild<Transform>("Bip001 L UpperArm", 0);
                builder.leftElbow = activeGameObject.transform.FindChild<Transform>("Bip001 L Forearm", 0);

                builder.rightArm = activeGameObject.transform.FindChild<Transform>("Bip001 R UpperArm", 0);
                builder.rightElbow = activeGameObject.transform.FindChild<Transform>("Bip001 R Forearm", 0);

                builder.middleSpine = activeGameObject.transform.FindChild<Transform>("Bip001 Spine1", 0);
                builder.head = activeGameObject.transform.FindChild<Transform>("Bip001 Head", 0);
        ...
        private void SetLayer()
        {
            foreach (BoneInfo bone in bones)
            {
                //bone.anchor.gameObject.layer = LayerMaskUtility.RAGDOLL_LAYER;
            }
        }
        ...
        void BuildBodies()
        {
            foreach (BoneInfo bone in bones)
            {
                Rigidbody rigidbody = Undo.AddComponent<Rigidbody>(bone.anchor.gameObject);
